	...
	if (isset($blockid)) {
		...
		<div class="panel panel-default">
			...
			<div class="panel-body">
				...
				<table class="table">
					...
					echo '<tr><td>'.lang("COIN_DESTROYED").'</td><td>'.TrimTrailingZeroes(number_format($coindaysdestroyed,8)).' '.lang("DAYS_DAYS").' => '.TrimTrailingZeroes(number_format($avgcoindaysdestroyed,8)).' <sup>'.lang("DAYS_DAYS").'</sup>/<sub>'.lang("COIN_COIN").'</sub></td></tr>
				</table>
		...
		<div class="panel panel-default">
			...
			<div class="panel-body">
				...
				<table class="table">
					<tr><th>'.lang("RECEIVED_TX").'</th><th>'.lang("TRACE_TRACE").'</th><th>'.lang("VALUE_VALUE").' [EMC]</th><th>'.lang("ADDRESS_ADDRESS").'</th><th>'.lang("COIN_DAYS").' ['.lang("DAYS_DAYS").']</th><th>'.lang("AVG_AGE").' [<sup>'.lang("DAYS_DAYS").'</sup>/<sub>'.lang("COIN_COIN").'</sub>]</th></tr>';
					...
					while($row_vin = $result_vin->fetch_assoc())
					{
						...
						if ($coinbase=="") {
							...
							echo '</td><td>'.TrimTrailingZeroes(number_format($value,8)).'</td><td>'.$address.'</td><td>'.TrimTrailingZeroes(number_format($coindaysdestroyed,2)).'</td><td>'.TrimTrailingZeroes(number_format($avgcoindaysdestroyed,2)).'</td></tr>';
						} else {
							echo '<tr><td>Coinbase</td><td>'.TrimTrailingZeroes(number_format($value,8)).'</td><td></td><td>N/A</td><td>'.TrimTrailingZeroes(number_format($coindaysdestroyed,2)).'</td><td>'.TrimTrailingZeroes(number_format($avgcoindaysdestroyed,2)).'</td></tr>';
						}
					}
				echo '</table>
